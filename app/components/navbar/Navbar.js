      ...
      <div className="container mx-auto flex items-center left-2">
        ...
        <Link href="/">
          <div className="items-center space-x-4 ml-5 flex">
            <img src="/homepage.png" alt="Logo" className="h-8 w-8 rounded-full hidden sm:flex" />
        ...
        <div className="flex items-center space-x-4 ml-auto">
          <button
            onClick={() => signOut()}
            className="text-white p-2 rounded-md hover:bg-white hover:text-black hidden sm:flex"
          >
            Logout
          </button>
          ...
          <Link href="/dashboard">
            <span className="text-white hidden sm:flex">{userEmail?.slice(0, userEmail.lastIndexOf("@"))}</span>
          </Link>
        </div>

        {/* Responsive Menu */}
        {isMenuOpen && (
          <div className="fixed left-0 top-[73px] right-0 bottom-0 bg-black bg-opacity-50 z-50">
